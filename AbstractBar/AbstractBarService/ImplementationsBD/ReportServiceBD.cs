...
namespace AbstractBarService.ImplementationsBD
{
    public class ReportServiceBD : IReportService
    {
        ...
        public void SaveCoctailPrice(ReportBindingModel model)
        {
            ...
            try
            {
                ...
                var products = context.Coctails.ToList();
                //создаем таблицу
                ...
                {
                    table.Cell(i + 1, 1).Range.Text = products[i].CoctailName;
        ...
        public List<CustomerRequestsModel> GetCustomerRequests(ReportBindingModel model)
        {
            return context.Requests
                            .Include(rec => rec.Customer)
                            .Include(rec => rec.Coctail)
                            .Where(rec => rec.DateCreate >= model.DateFrom && rec.DateCreate <= model.DateTo)
                            .Select(rec => new CustomerRequestsModel
                            {
        ...
        public void SaveCustomerRequests(ReportBindingModel model)
        {
            ...
            //заполняем таблицу
            var list = GetCustomerRequests(model);
            ...
            for (int i = 0; i < list.Count; i++)
            {
                ...
                table.AddCell(cell);
                cell = new PdfPCell(new Phrase(list[i].CoctailName, fontForCells));
                table.AddCell(cell);
